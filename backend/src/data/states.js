/** All 50 states + DC + Puerto Rico, with FIPS codes. Census/FRED share these codes. */
export const STATES = [
  ...
  ["72", "PR", "Puerto Rico"],
].map(([fips, abbr, name]) => ({ fips, abbr, name }));

export const STATE_BY_FIPS = new Map(STATES.map((s) => [s.fips, s]));
export const STATE_BY_ABBR = new Map(STATES.map((s) => [s.abbr.toUpperCase(), s]));
export const STATE_BY_NAME = new Map(STATES.map((s) => [s.name.toLowerCase(), s]));

/** Accepts "SC", "South Carolina", or "45". */
export function resolveState(input) {
  if (!input) return null;
  const raw = String(input).trim();
  return (
    STATE_BY_FIPS.get(raw.padStart(2, "0")) ||
    STATE_BY_ABBR.get(raw.toUpperCase()) ||
